...
class MyModule:
    ...
    def add_parameter(self, shape):
        # Tạo một tham số ngẫu nhiên và thêm vào danh sách
        parameter = [0.0] * shape
        self.parameters.append(parameter)

    def forward(self, input_data):
        # Đây là phương thức forward đơn giản, bạn có thể tùy chỉnh theo ý muốn
        output = input_data
        for parameter in self.parameters:
            output = [x + w for x, w in zip(output, parameter)]
        return output

# Sử dụng MyModule
model = MyModule()
model.add_parameter(3)  # Thêm một tham số có kích thước 3
model.add_parameter(3)  # Thêm một tham số khác có kích thước 3
